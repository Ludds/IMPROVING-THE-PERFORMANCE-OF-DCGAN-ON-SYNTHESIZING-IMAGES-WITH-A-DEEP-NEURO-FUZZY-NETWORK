...
def createBatch(dReal, lReal, dFake, lFake):
	input = np.append(dReal, dFake, axis=0)
	lable = np.append(lReal, lFake, axis=0)

	indices = np.arange(start=0, stop=setting.batch_size*2, step=2)
	np.random.shuffle(indices)
	return input[indices], lable[indices]
